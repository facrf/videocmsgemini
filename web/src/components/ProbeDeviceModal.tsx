...
export const ProbeDeviceModal: React.FC<ProbeDeviceModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-lg w-full overflow-hidden shadow-2xl flex flex-col">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Sondar e Cadastrar Câmera</h3>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Usuário ONVIF</label>
                <input
                  type="text"
                  value={username}
            ...
            <button
              ...
              onClick={handleProbe}
              disabled={probing}
              className="w-full py-2 text-xs font-medium bg-slate-800 hover:bg-slate-700 text-blue-400 rounded-lg border border-slate-700 flex items-center justify-center gap-2 transition disabled:opacity-50"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${probing ? 'animate-spin-custom' : ''}`} />
          ...
          {/* Probed Details */}
          {probedResult && (
            <div className="p-3 bg-slate-950/60 border border-slate-800 rounded-lg text-xs space-y-1 font-mono">
              ...
              {probedResult.manufacturer && <div>Fabricante: <span className="text-white">{probedResult.manufacturer}</span></div>}
              {probedResult.model && <div>Modelo: <span className="text-white">{probedResult.model}</span></div>}
